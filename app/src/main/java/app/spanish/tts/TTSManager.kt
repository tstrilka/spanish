...
class TTSManager @Inject constructor(
    ...
    init {
        if (!isTTSAvailable()) {
            ...
            app.spanish.util.Logger.e("TTS not available")
        } else {
            initializeTTS()
        }
    }
    ...
    private fun isTTSAvailable(): Boolean {
        return try {
            ...
            app.spanish.util.Logger.e("TTS availability check failed", e)
            false
        }
    }
        ...
        try {
            tts = TextToSpeech(context) { status ->
                if (status == TextToSpeech.SUCCESS) {
                    ...
                    app.spanish.util.Logger.e("TTS initialization failed with status: $status")
                }
            }
        } catch (e: Exception) {
            _error.value = "Failed to create TTS: ${e.message}"
            app.spanish.util.Logger.e("TTS creation failed", e)
        }
    }
        ...
        tts?.let { ttsEngine ->
            ...
            when (result) {
                ...
                TextToSpeech.LANG_COUNTRY_VAR_AVAILABLE -> {
                    ...
                    if (setResult == TextToSpeech.LANG_MISSING_DATA || setResult == TextToSpeech.LANG_NOT_SUPPORTED) {
                        ...
                        app.spanish.util.Logger.e("Spanish language not supported")
                    } else {
                        _isReady.value = true
                        _error.value = null
                        app.spanish.util.Logger.i("Spanish TTS ready")
                    }
                }
                    ...
                    for (locale in fallbackLocales) {
                        if (ttsEngine.isLanguageAvailable(locale) >= TextToSpeech.LANG_AVAILABLE) {
                            ...
                            if (setResult != TextToSpeech.LANG_MISSING_DATA && setResult != TextToSpeech.LANG_NOT_SUPPORTED) {
                                ...
                                _error.value = null
                                configured = true
                                app.spanish.util.Logger.i("Fallback Spanish TTS ready: $locale")
                                break
                            }
                        }
                    }
                    ...
                    if (!configured) {
                        ...
                        app.spanish.util.Logger.e("No Spanish TTS available")
                    }
                }
            }
            ...
            ttsEngine.setOnUtteranceProgressListener(object : UtteranceProgressListener() {
                override fun onStart(utteranceId: String?) {
                    app.spanish.util.Logger.d("TTS started: $utteranceId")
                }

                override fun onDone(utteranceId: String?) {
                    app.spanish.util.Logger.d("TTS completed: $utteranceId")
                }

                override fun onError(utteranceId: String?) {
                    app.spanish.util.Logger.e("TTS error: $utteranceId")
    ...
    suspend fun speak(text: String) {
        withContext(Dispatchers.Main) {
            ttsLock.withLock {
                ...
                try {
                    if (!isTTSHealthy()) {
                        app.spanish.util.Logger.w("TTS not healthy, reinitializing")
                        reinitializeTTS()
                        return@withLock
                    }
                    ...
                    if (result == TextToSpeech.ERROR) {
                        ...
                        app.spanish.util.Logger.e("TTS speak failed")
                    } else {
                        _error.value = null
                        app.spanish.util.Logger.i("Speaking: $text")
                    }
                } catch (e: Exception) {
                    _error.value = "TTS error: ${e.message}"
                    app.spanish.util.Logger.e("TTS speak exception", e)
                }
            }
        }
    }
    ...
    private fun reinitializeTTS() {
        app.spanish.util.Logger.i("Reinitializing TTS")
        stop()
        shutdown()
    ...
    fun stop() {
        try {
            ...
            app.spanish.util.Logger.e("TTS stop failed", e)
        }
    }
    ...
    fun shutdown() {
        ...
        try {
            ...
            app.spanish.util.Logger.e("TTS shutdown failed", e)
        }
    }
}